...
export type CreateDemoSandboxResult =
  | { status: 'created' | 'resumed'; sandbox: DemoSandbox }
  | { status: 'full'; liveSandboxes: number; limit: number }
  | { status: 'template-missing' }

/** Deletes every Cloud Storage object under a prefix. Injected so tests can watch it. */
export type DeleteStoragePrefix = (prefix: string) => Promise<void>

function sessionsCollection(db: Firestore) {
  return db.collection(DEMO_SESSIONS_COLLECTION)
}

function templateCampaignRef(db: Firestore) {
  return db
    .collection('groups')
...
class BatchWriter {
  ...
  async flush() {
    if (this.pending > 0) this.commits.push(this.batch.commit())
...
export async function createDemoSandbox(
  ...
  uid: string,
  nowMs: number,
): Promise<CreateDemoSandboxResult> {
 ...
 */
export async function sweepExpiredDemoSandboxes(
  db: Firestore,
  deleteStoragePrefix: DeleteStoragePrefix,
  nowMs: number,
): Promise<{ removedGroupIds: string[] }> {
  ...
  for (const sessionSnap of expired.docs) {
    const groupId = typeof sessionSnap.data().groupId === 'string' ? (sessionSnap.data().groupId as string) : ''
    if (groupId) {
