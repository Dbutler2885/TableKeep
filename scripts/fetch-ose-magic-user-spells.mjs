...
function parseIndexHtml(html) {
  ...
  for (const [, slug, titleAttr, innerHtml] of links) {
    const candidateName = normalizeSpellName(stripHtml(innerHtml) || titleAttr)
    const titleName = normalizeSpellName(titleAttr)
...
async function fetchSpellPage(slug) {
  const url = `${API_BASE}?action=parse&page=${encodeURIComponent(slug)}&prop=text&format=json&formatversion=2`
